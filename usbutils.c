...
// used here only for Antminer S7 atm
...
int _usb_transfer_read(struct cgpu_info *cgpu, uint8_t request_type, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, char *buf, unsigned int bufsiz, int *amount, unsigned int timeout, __maybe_unused enum usb_cmds cmd)
{
	struct cg_usb_device *usbdev;
#if DO_USB_STATS
	struct timeval tv_start, tv_finish;
...
int usb_ftdi_cts(struct cgpu_info *cgpu)
{
	...
	ret = (char) (buf[0] & FTDI_STATUS_B0_MASK);
