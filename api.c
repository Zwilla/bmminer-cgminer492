...
#ifdef HAVE_AN_ASIC // only for S7
...
static void poolstatus(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	...
	//  root = api_add_time(root,   "Last Share Time", &(pool->last_share_time), false); // 1472763377
		root = api_add_string(root, "Last Share Time", lasttime, false); // 00:00:03
		...
		double rejp = (pool->diff_accepted + pool->diff_rejected + pool->diff_stale) ?
                      (pool->diff_rejected) / (pool->diff_accepted + pool->diff_rejected + pool->diff_stale) : 0;

		root = api_add_percent(root, "Pool Rejected%", &rejp, false);

		double stalep = (pool->diff_accepted + pool->diff_rejected + pool->diff_stale) ?
                        (pool->diff_stale) / (pool->diff_accepted + pool->diff_rejected + pool->diff_stale) : 0;
...
static void summary(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	...
	double prejp = (total_diff_accepted + total_diff_rejected + total_diff_stale) ?
                   (total_diff_rejected) / (total_diff_accepted + total_diff_rejected + total_diff_stale) : 0;
	...
	double stalep = (total_diff_accepted + total_diff_rejected + total_diff_stale) ?
                    (total_diff_stale) / (total_diff_accepted + total_diff_rejected + total_diff_stale) : 0;
		...
		else
        {
            ...
            if (*slash)
            {
				*(slash++) = '\0';
                ...
                {
					applog(LOG_ERR, "API: ignored address with invalid mask (%d) '%s'", mask, original);
			...
			if (ipv6)
            {
                ...
                {
					applog(LOG_ERR, "API: ignored invalid IPv6 address '%s'", original);
					goto popipo;
                }
			}
			else
            {
                ...
                {
					applog(LOG_ERR, "API: ignored invalid IPv4 address '%s' (as %s)", original, tmp);
					goto popipo;
			}
			}
	...
	*group = NOPRIVGROUP;
	...
	if (opt_api_allow)
    {
		for (i = 0; i < ips; i++)
        {
			...
			for (j = 0; j < 16; j++)
            {
				if ((client_ip.s6_addr[j] & ipaccess[i].mask.s6_addr[j]) != ipaccess[i].ip.s6_addr[j])
                {
					match = false;
					break;
				}
    ...
    else
    {
        ...
        {
            addrok = (strcmp(*connectaddr, localaddr) == 0) || IN6_IS_ADDR_LOOPBACK(&client_ip);
        }
	}
	...
	while (80085)
    {
		...
		if ((size_t)rep > expect_code_len && memcmp(buf, expect_code, expect_code_len) == 0)
        {
            ...
            else
            {
				...
				snprintf(replybuf, sizeof(replybuf), "cgm-" API_MCAST_CODE "-%d-%s", opt_api_port, opt_api_mcast_des);

				rep = sendto(reply_sock, replybuf, strlen(replybuf)+1, 0, client->ai_addr, client->ai_addrlen);
				freeaddrinfo(res);
